...
async def tgscmd(message):
    ...
    if not reply:
        await edl(message, "`Reply to an animated sticker`", 3)
        return
    if not reply.file.name.endswith(".tgs"):
        await edl(message, "`Reply to an animated sticker`", 3)
        return
    await reply.download_media("tgs.tgs")
    await eor(message, "`Fixing this sticker...`")
    system("lottie_convert.py tgs.tgs json.json")
    ...
    system("lottie_convert.py json.json tgs.tgs")
