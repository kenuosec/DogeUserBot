...
@doge.bot_cmd(
    ...
    info={
        ...
        "usage": "{tr}yt [URL / Text] or [Reply to URL / Text]",
    },
)
async def yt_inline(event):
    ...
    while flag:
        try:
            results = await event.client.inline_query(
                Config.BOT_USERNAME, f"yt {input_url}"
            )
            flag = False
    ...
    if results:
        ...
        await dogevent.edit("**🚨 Sorry! I can't find any results.**")
...
async def ytdl_download_callback(c_q: CallbackQuery):  # sourcery no-metrics
    ...
    _fpath = ""
    thumb_pic = None
    for _path in glob(path.join(Config.TEMP_DIR, str(startTime), "*")):
        if _path.lower().endswith((".jpg", ".png", ".webp")):
            thumb_pic = _path
        else:
            _fpath = _path
    if not _fpath:
        await edl(upload_msg, "**🙁 I'm not found anyhing!**")
        return
    if not thumb_pic:
...
async def ytdl_callback(c_q: CallbackQuery):
    ...
    if choosen_btn == "back":
        ...
        telegraph = await post_to_telegraph(
            f"ℹ️ Showing {total} YouTube video results for the given query...",
            list_res,
        )
        await c_q.edit(
